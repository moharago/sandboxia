...
const sandboxAnalysis: AIAnalysisData = {
  ...
  confidence: 87,
  summary:
    "본 서비스는 현행 규제 체계에서 즉시 시장 출시가 어려우며, 규제 샌드박스를 통한 실증이 필요합니다.",
  reasons: [
    {
      category: "law",
      title: "규제 저촉 사항 발견",
      description:
        "자율주행 배달로봇의 보도 운행은 현행법상 명확한 법적 근거가 부재하며, 도로 운행 시 운전자 탑승 의무 조항에 저촉됩니다.",
      source:
    ...
    },
    {
      ...
      title: "유사 승인 사례 존재",
      description:
        "동일 서비스 유형의 실증특례 승인 선례가 있어 승인 가능성이 높으나, 안전성 검증 자료 보강이 필요합니다.",
      source:
...
const directAnalysis: AIAnalysisData = {
  ...
  confidence: 94,
  summary:
    "본 서비스는 현행 규제 체계 내에서 별도의 규제 특례 없이 즉시 시장 출시가 가능합니다.",
  reasons: [
    {
      category: "law",
      title: "규제 저촉 사항 없음",
      description:
        "대형 시설 내 자율주행 청소 로봇은 「도로교통법」 적용 대상이 아니며, 사유지 내 운행으로 별도 허가가 불필요합니다.",
      source:
        "「도로교통법」 제2조 (정의), 「산업안전보건법」 제93조 (안전인증 대상)",
    },
    {
      category: "regulation",
      title: "기존 인허가 체계 적용 가능",
      description:
        "전기용품 안전인증(KC) 및 전자파 적합성 인증만 취득하면 현행법 내에서 서비스 제공이 가능합니다.",
    ...
    },
    {
      ...
      description:
        "LG전자, 네이버랩스 등 다수 기업이 동일 유형의 서비스를 규제 특례 없이 상업 운영 중입니다.",
      source:
        "LG CLOi 로봇 시리즈 상용화 사례, 네이버랩스 루키(Rookie) 서비스 운영 사례",
    },
  ],
  directLaunchRisks: [],
};
...
export default function MarketPage({ params }: MarketPageProps) {
  ...
  return (
    <div className="py-6">
      {isSaving && (
        <AILoadingOverlay
          message={selectedDecision === "direct" ? "저장 중" : "AI 분석 중"}
        />
      )}
      <div className="container mx-auto px-4 space-y-6">
        <div>
          ...
          <p className="text-muted-foreground">
            AI가 서비스의 규제 현황을 분석하여 시장 출시 가능 여부를 판단합니다
          </p>
        </div>
                ...
                return (
                  ...
                  >
                    <Badge
                      variant="outline"
                      className={cn(
                        "shrink-0 mt-0.5 text-xs font-medium",
                        config.badgeClass
                      )}
                    >
                      {config.label}
            ...
            {selectedDecision !== analysisData.recommendation && (
              ...
              <div className="text-sm flex gap-2 items-center px-2">
                ...
                <span className="text-amber-700">
                  AI 추천과 다른 선택입니다. 선택에 대한 근거를 확인해주세요.
                </span>
              </div>
            )}
          </CardContent>
