...
import { CASE_STATUS_LABELS, CASE_STAGE_LABELS, SANDBOX_TYPE_LABELS } from "@/types/data/case"
                ...
                <div className="flex items-center gap-4 p-4 transition-colors hover:bg-muted/30 group">
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                            ...
                            <Badge variant={statusBadgeVariant[caseData.status]} className="shrink-0">
                                {CASE_STATUS_LABELS[caseData.status]}
                            </Badge>
                        </div>
                        <p className="text-sm text-muted-foreground truncate">{caseData.service}</p>
                    </div>
                    <div className="hidden sm:flex items-center gap-6 text-sm text-muted-foreground">
                        {caseData.sandboxType && <span>{SANDBOX_TYPE_LABELS[caseData.sandboxType]}</span>}
                        ...
                        <div className="flex items-center justify-between text-sm">
                            <div>
                                {caseData.sandboxType && (
                                    <span className="px-2 py-1 bg-muted rounded text-xs">{SANDBOX_TYPE_LABELS[caseData.sandboxType]}</span>
                                )}
                            </div>
                            <span className="font-medium">{caseData.progress}%</span>
                        </div>
                        <div className="h-1 w-full bg-gray-200 rounded-full overflow-hidden mb-0">
                            <div className="h-full bg-teal-600 transition-all duration-300 ease-in-out" style={{ width: `${caseData.progress}%` }} />
                        </div>
