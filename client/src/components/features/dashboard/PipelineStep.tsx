...
export function PipelineStep({ step, label, count, isActive = false, onClick, className }: PipelineStepProps) {
    return (
        <button
            type="button"
            onClick={onClick}
            className={cn("flex flex-col items-center text-center group cursor-pointer relative w-10 shrink-0", className)}
        >
            <div
                className={cn(
...
export function Pipeline({ steps, activeFilter, onFilterChange }: PipelineProps) {
    return (
