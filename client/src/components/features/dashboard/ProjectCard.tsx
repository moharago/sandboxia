...
import { PROJECT_STATUS_LABELS, PROJECT_STAGE_LABELS, SANDBOX_TYPE_LABELS } from "@/types/data/project"
...
export function ProjectCard({ project, viewMode = "grid" }: ProjectCardProps) {
    const formattedDate = new Date(project.updatedAt)
                ...
                <div className="flex items-center gap-4 p-4 transition-colors hover:bg-muted/30 group">
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                            <h3 className="font-medium truncate">{project.company}</h3>
                            <Badge variant={statusBadgeVariant[project.status]} className="shrink-0">
                                {PROJECT_STATUS_LABELS[project.status]}
                            </Badge>
                        </div>
                        <p className="text-sm text-muted-foreground truncate">{project.service}</p>
                    </div>
                    <div className="hidden sm:flex items-center gap-6 text-sm text-muted-foreground">
                        {project.sandboxType && <span>{SANDBOX_TYPE_LABELS[project.sandboxType]}</span>}
            ...
            <Card className="hover:shadow-lg transition-all h-full flex flex-col">
                <CardHeader className="pb-3">
                    <div className="flex items-start justify-between gap-2 overflow-hidden">
                        <h3 className="font-semibold truncate">{project.company}</h3>
                        <Badge variant={statusBadgeVariant[project.status]} className="shrink-0">
                            {PROJECT_STATUS_LABELS[project.status]}
                        </Badge>
                    </div>
                ...
                <CardContent className="pb-3 flex-1">
                    <div className="space-y-3">
                        <div className="flex items-center justify-between text-sm">
                            <div>
                                {project.sandboxType && (
                                    <span className="px-2 py-1 bg-muted rounded text-xs">{SANDBOX_TYPE_LABELS[project.sandboxType]}</span>
                                )}
                            </div>
                            <span className="font-medium">{project.progress}%</span>
                        </div>
                        <div className="h-1 w-full bg-gray-200 rounded-full overflow-hidden mb-0">
                            <div className="h-full bg-teal-600 transition-all duration-300 ease-in-out" style={{ width: `${project.progress}%` }} />
                        </div>
