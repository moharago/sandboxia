...
- Top-K 비교: "top-k 5, 10, 15 비교", "k=5랑 k=10 비교해줘"
- **프리셋 비교**: "C1~C3 각각 평가", "C1, C2, C3 비교", "청킹 프리셋 비교", "E0~E2 비교"
- 조합 비교: "C1 E0, C1 E1 비교", "여러 설정으로 테스트"

→ Task tool로 `rag-evaluator` 에이전트를 **병렬 호출**
→ 각 설정별 서브에이전트가 **수집 + VectorDB + 평가** 실행 (모드 A+ 동작)
...
**프리셋 범위/목록 패턴** (모드 B 트리거 - 병렬 평가):

- "C1~C3 각각 평가", "C0~C6 비교" → C1, C2, C3 각각 병렬 실행
- "E0~E2 비교", "E1, E2 비교" → E0, E1, E2 각각 병렬 실행
- "C1, C2, C3 평가" → 쉼표로 구분된 프리셋 각각 병렬 실행

프리셋 **범위(~) 또는 목록(,)**이 감지되면 **모드 B**로 전환합니다.
...
→ Task tool 3개 병렬 호출 (각각 수집 + VectorDB + 평가 실행):
  - prompt: "R3 C1 프리셋 평가 실행. 수집부터 평가까지 전체 실행. output=2026-02-12_chunking_C1"
  - prompt: "R3 C2 프리셋 평가 실행. 수집부터 평가까지 전체 실행. output=2026-02-12_chunking_C2"
  - prompt: "R3 C3 프리셋 평가 실행. 수집부터 평가까지 전체 실행. output=2026-02-12_chunking_C3"
...
→ Task tool 2개 병렬 호출:
  - prompt: "R3 E0 프리셋 평가 실행. 수집부터 평가까지 전체 실행. output=2026-02-12_embedding_E0"
  - prompt: "R3 E1 프리셋 평가 실행. 수집부터 평가까지 전체 실행. output=2026-02-12_embedding_E1"
...
| `/rag-eval R3 C1~C3 각각 평가` | B    | **프리셋별 수집+평가 병렬** → 비교표   | `chunking_C1~C3.json`        |
| `/rag-eval R3 E0~E1 비교`      | B    | 임베딩 프리셋별 병렬 평가              | `embedding_E0~E1.json`       |
...
- **프리셋 병렬 평가 (모드 B)**: C1~C3 같은 범위 평가 시 각 프리셋별로 **순차적으로** 수집+평가 실행 (Vector DB 공유로 인해 병렬 불가). 총 소요시간 = 프리셋 수 × 단일 평가 시간
