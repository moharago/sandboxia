...
> "⚠️ {RAG_TYPE} 평가 스크립트가 아직 구현되지 않았습니다. 현재 R3만 사용 가능합니다."
...
```bash
cd server && uv run python eval/{rag_type}/run_{type}.py --help
```
...
에이전트 레퍼런스: `.claude/agents/rag-evaluator.md`
