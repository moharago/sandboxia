...
def _get_config_paths(rag_type: str = "r3") -> tuple[Path, Path]:
    """RAG 타입별 설정 파일 경로 반환"""
...
def _load_chunking_presets(rag_type: str = "r3") -> dict:
    """청킹 프리셋 로드"""
    chunking_path, _ = _get_config_paths(rag_type)
    return _load_yaml(chunking_path)


def _load_embedding_presets(rag_type: str = "r3") -> dict:
    """임베딩 프리셋 로드"""
    _, embedding_path = _get_config_paths(rag_type)
...
def load_config(config_name: str, rag_type: str = "r3") -> ChunkingConfig | EmbeddingConfig:
    """설정 이름에 따라 청킹(C*) 또는 임베딩(E*) 설정 로드

    Args:
        config_name: 설정 이름 (C*는 청킹, E*는 임베딩)
        rag_type: RAG 타입 (r1, r2, r3)

    Returns:
        ChunkingConfig 또는 EmbeddingConfig 인스턴스
    """
    if config_name.upper().startswith("E"):
        return load_embedding_config(config_name.upper(), rag_type)
    return load_chunking_config(config_name.upper(), rag_type)
