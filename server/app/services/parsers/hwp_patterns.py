...
PLAN_COMMON_PATTERNS: dict[str, list[str]] = {
    ...
    "expected_quantitative": [
        r"가\.\s*정량적\s*기대효과[:\s]*(.+?)(?:나\.|$)",
    ],
    "expected_qualitative": [
        r"나\.\s*정성적\s*기대효과[:\s]*(.+?)(?:6\.|$)",
    ],
    "expansion_plan": [
...
DEMONSTRATION_APPLICATION_PATTERNS: dict[str, list[str]] = {
    "regulatory_exemption_reason_1": [
        r"법\s*제38조의2제1항제1호",
        r"허가\s*등을\s*신청하는\s*것이\s*불가능한\s*경우",
    ],
    "regulatory_exemption_reason_2": [
        r"법\s*제38조의2제1항제2호",
        r"불명확하거나\s*불합리한\s*경우",
    ],
}
