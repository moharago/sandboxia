...
class DocumentSubtype(str, Enum):
    ...
    TEMPORARY_JUSTIFICATION = "temporary-3"  # 신청사유
    ...
    DEMONSTRATION_JUSTIFICATION = "demonstration-3"  # 신청사유
...
# 참고: [\s\S]+?로 여러 줄 캡처, 가이드 텍스트는 _remove_guide_text에서 제거
# ============================================================
FASTCHECK_DESCRIPTION_PATTERNS: dict[str, list[str]] = {
    "technology_service_details": [
        # 섹션 전체 캡처 (가이드 텍스트 포함) → _remove_guide_text에서 정리
        r"1\.\s*기술[·‧\s]*서비스\s*세부내용\s*([\s\S]+?)(?=\n2\.\s*법[·‧\s]*제도|\Z)",
    ],
    "legal_issues": [
        r"2\.\s*법[·‧\s]*제도\s*이슈\s*사항\s*([\s\S]+?)(?=\n3\.\s*기타\s*질의|\Z)",
    ],
    "additional_questions": [
        r"3\.\s*기타\s*질의\s*사항\s*([\s\S]+?)(?=\Z)",
    ],
    # 기술 정보 (설명서용 - 별도 섹션)
    "core_technology": [
        r"주요\s*기술\s*요소[:\s]*([\s\S]+?)(?=\n\n|2\.\s*|\Z)",
        r"핵심\s*기술[:\s]*([\s\S]+?)(?=\n\n|\Z)",
    ],
    "regulatory_issues": [
        r"법령\s*해석\s*관련\s*이슈[:\s]*([\s\S]+?)(?=\n\n|②|\Z)",
        r"규제\s*사안[:\s]*([\s\S]+?)(?=\n\n|\Z)",
    ],
    "related_laws": [
        r"관련\s*법령\s*체계[^:]*[:\s]*([\s\S]+?)(?=\n\n|①|\Z)",
        r"관련\s*법령[:\s]*([\s\S]+?)(?=\n\n|\Z)",
    ],
}
...
PLAN_COMMON_PATTERNS: dict[str, list[str]] = {
    ...
    # 섹션 텍스트 필드 (여러 줄 캡처: [\s\S]+?)
    # 참고: HWP는 U+2027(‧)을 사용하므로 [·‧]로 둘 다 매칭
    # 참고: MULTILINE 모드에서 $는 줄 끝을 매칭하므로 \Z(문자열 끝)를 사용
    "detailed_description": [
        r"가\.\s*기술[·‧\s]*서비스\s*세부\s*내용([\s\S]+?)(?=나\.\s*기술[·‧\s]*서비스\s*관련|\Z)",
        r"1\.\s*기술[·‧\s]*서비스\s*내용([\s\S]+?)(?=2\.\s*|\Z)",
    ],
    "market_status": [
        r"나\.\s*기술[·‧\s]*서비스\s*관련\s*시장\s*현황\s*및\s*전망([\s\S]+?)(?=2\.\s*임시허가|2\.\s*규제특례|\Z)",
    ],
    "regulation_details": [
    ...
    "business_content": [
        r"나\.\s*사업\s*내용([\s\S]+?)(?=다\.\s*사업|\Z)",
    ],
    "execution_method": [
        r"나\.\s*단계별\s*추진\s*방법([\s\S]+?)(?=다\.\s*|\Z)",
    ],
    "schedule": [
        r"다\.\s*(?:사업|실증)\s*기간\s*및\s*일정\s*계획([\s\S]+?)(?=4\.\s*|\Z)",
    ],
    "operation_plan": [
        r"4\.\s*(?:사업|실증)\s*운영\s*계획([\s\S]+?)(?=5\.\s*기대효과|\Z)",
    ],
    "expected_quantitative": [
        r"가\.\s*정량적\s*기대효과([\s\S]+?)(?=나\.\s*정성적|\Z)",
    ],
    "expected_qualitative": [
        r"나\.\s*정성적\s*기대효과([\s\S]+?)(?=6\.\s*사업|6\.\s*실증|\Z)",
    ],
    "expansion_plan": [
        r"6\.\s*(?:사업\s*확대[·‧\s]*확산|실증\s*이후)\s*계획([\s\S]+?)(?=7\.\s*|붙임|\Z)",
    ],
    "restoration_plan": [
        r"나\.\s*실증\s*후\s*복구\s*계획([\s\S]+?)(?=7\.\s*|\Z)",
    ],
    "organization_structure": [
        r"가\.\s*추진\s*체계([\s\S]+?)(?=나\.\s*추진\s*예산|\Z)",
    ],
    "budget": [
        r"나\.\s*추진\s*예산([\s\S]+?)(?=붙임|\Z)",
    ],
    # 붙임 1. 신청기관 현황자료
    "establishment_date": [
        r"설\s*립\s*일[\s:]*\n*(.+?)(?:\n대\s*표\s*자|\n|$)",
...
SAFETY_PROTECTION_PATTERNS: dict[str, list[str]] = {
    "safety_verification": [
        r"1\.\s*안전성\s*검증\s*자료([\s\S]+?)(?=2\.\s*이용자|\Z)",
    ],
    "user_protection_plan": [
        r"2\.\s*이용자\s*보호\s*(?:및\s*)?대응\s*(?:계획)?([\s\S]+?)(?=3\.\s*|\Z)",
    ],
    "risk_and_response": [
        r"3\.\s*(?:임시허가|규제특례)에\s*따른\s*위험\s*및\s*대응\s*방안([\s\S]+?)(?=4\.\s*|\Z)",
    ],
    "stakeholder_conflict": [
        r"4\.\s*기존\s*시장\s*및\s*이용자\s*등의\s*이해관계\s*충돌[^:]*[:\s]*([\s\S]+?)(?=\Z)",
    ],
}
