...
HWP의 "재무상태" 또는 "재무현황" 테이블에서 **모든 행**의 숫자 데이터를 추출하세요:
- 테이블 컬럼: M-2년도, M-1년도, 평균
- 테이블 행 (반드시 모두 추출): 총자산, 자기자본, 유동부채, 고정부채, 유동자산, 당기순이익, **총매출액**, **자기자본 이익률**, 부채비율
- 총매출액, 자기자본 이익률, 부채비율 행이 테이블에 있으면 반드시 추출하세요!
- 퍼센트(%) 값은 숫자만 추출 (예: "10.0%" → 10.0)
...
raw_text의 "재무상태" 또는 "재무현황" 테이블에서 **모든 행**의 숫자 데이터를 추출하세요:
- 테이블 컬럼: M-2년도, M-1년도, 평균
- 테이블 행 (반드시 모두 추출): 총자산, 자기자본, 유동부채, 고정부채, 유동자산, 당기순이익, **총매출액**, **자기자본 이익률**, 부채비율
- 퍼센트(%) 값은 숫자만 추출 (예: "10.0%" → 10.0)
